...
2. Move into the `TorQ-Amazon-FinSpace-Starter-Pack/terraform-deployment` directory; within there should be two sub-directories : `date-par` and `int-par`. 
...
7. From your Terraform working directory which is `TorQ-Amazon-FinSpace-Starter-Pack/terraform-deployment/date-par|int-par`, run `terraform init`.
