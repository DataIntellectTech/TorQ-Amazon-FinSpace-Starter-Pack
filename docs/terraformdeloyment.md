...
2. Move into the `TorQ-Amazon-FinSpace-Starter-Pack/terraform-deployment` directory; within there should be two sub-directories : `default` and `int-par`. 
  If you want your finspace with Managed Kdb Insights system to use date partitions use `default`
  If you want your system to perform intra-day writedowns with int partitions use `int-par` 
  Choose one to be the the Terraform working directory from which you should run all `terraform` commands.
...
7. From your Terraform working directory which is `TorQ-Amazon-FinSpace-Starter-Pack/terraform-deployment/default|int-par`, run `terraform init`.
